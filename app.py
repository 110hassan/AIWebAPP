...
def predict():
    ...
    with sqlite3.connect("aiwebapp.db") as reports: 
        cur = reports.cursor()
        cur.execute("INSERT INTO reports (Name, Age, Gender, Email, Place, Date, Time, Diagnosis, Interpretations, Recommendations, Image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (name,age,gender,contact,place,today,clock,caption,interpretation,recommendation,imagePath))
        reports.commit()
